...
### Section AM — 2026-07-13 (Session recon + first post-gate reflection change + strategic freeze decision)

#### AM.1 — Sessions 07-10 and 07-13 analyzed; broker reconciliation flipped 07-13 negative
- **07-10: net +₹819.35**, dominated by S22 BTST exits (BHEL +4368.80, ADANIENT +465.00, VEDL −2190.10, MUTHOOTFIN −1843.20 = +800.50 net); DeepSeek intraday leg +18.85 across 5 closes. Broker export `(20)` matched the CSV exactly — no reconciliation needed.
- **07-13: bot logged +7.39 (4W/4L), broker truth is −₹3.15 (3W/7L)**. Reconciled `signals_2026-07-13.csv` to `Closed-P&L-All (21).csv` per the AL.1 convention: **ASTRAL sign flip** +0.20 → **−0.20**; NMDC 9.63 → **9.09**; BIOCON 6.20 → **4.60** (bot short leg only; the manual EOD long's +2.40 has no signal row — out of scope); HINDCOPPER −0.05 → −0.10; TATAPOWER −2.10 → −3.00; PIDILITIND −3.90 → −3.80; filled **DMART −6.30** and **INDUSINDBK −0.85** (broker-side super-order SL fills at ~09:59 — the bot only logged "no longer in Dhan positions", no exit row: recurring blind spot). JUBLFOOD left blank (DH-906 rejection, never filled). Zero targets hit again — every close was trailing-SL/reversal (exit-geometry leak persists).
- **Operational findings 07-13**: (a) **S22 runner was never started** (Monday session missed entirely; `s22_overnight_state.json` empty, no weekend carry) — the one validated strategy missed a session while the unvalidated one ran fine; the 07-10 S22 *intraday* legs also have no exit rows in the CSV. (b) **BIOCON guardian re-discovery race at EOD**: a manual long was "closed" 3× in 60s by three different exit paths (REV-VOLUME-CLIMAX → position_exit_guardian full-exit → KRONOS-EXIT), the guardian re-adding the position from stale Dhan sync after each close. No net damage this time (broker BIOCON total +7.00) but must be fixed before real size. (c) One MarketFeed WS keepalive drop, recovered.

#### AM.2 — analog_history.db re-seeded + backfill
Local `analog_history.db` did not exist (bot lives on the droplet now) — `backfill_rag.py` fully re-seeded it from all signal CSVs (174 rows). After reconciliation, purged the 8 stale 07-13 rows (backfill never updates pnl on existing keys) and re-ran `--date 2026-07-13` → 10 rows with broker-true PnL. **DB = 176 rows.** Note: local candle store (`kronos_integrated_bot/data/`) ends 2026-07-01, so replay validation can no longer see recent days until candles are synced from the droplet.

#### AM.3 — Reflection: evidence gate opened, first change since 06-30 → kronos-v21 (with caveats)
Gate opened at 38 ≥ 30 closed trades. Kept `min_volume_ratio_trending`, then proposed and applied **`min_adx_trending` 22 → 24** (v20 archived → **kronos-v21**). Hypothesis: ADX 25-40 bucket 25% WR / −33 PnL vs >40 bucket 66.7% WR. Replay delta was noise (+0.60 over 23 trades) and only covered 06-24..07-01. **Two evidence problems**: (1) `reflect.py` has **no mode filter** — S22/BTST rupee PnL (hundreds of shares) contaminates the unweighted stats vs qty-1 canary trades; the "avg +72/trade" that passed the prior hypothesis is entirely S22 while DeepSeek-leg WR fell to ~34%. Fix = exclude `LIVE-S22%` modes or evaluate on pnl_pct. (2) Reflection ran **before** the broker reconciliation (saw 07-13 as +7.39/4W, truth −3.15/3W). Change left in place — conservative either way — to be judged at the next gate.

#### AM.4 — EMA/RSI lab study: another pre-registered negative (0 survivors)
`intraday_lab/ema_rsi_study.py` (+ `strategies/ema_rsi.py`, tests): EMA-cross+RSI long/short, plain and filtered, 3-min and 5-min, net of costs, IS/OOS + walk-forward. **All variants REJECTED** (best OOS PF 1.42 on longs but WF 3/9 folds+, shorts OOS PF 0.47). Consistent with the AK/lab verdict: no mineable 5-min entry edge. Also fixed a real bug in `strategies/base.py` `supertrend_dir`: ATR warm-up NaNs froze the bands/direction forever (NaN comparisons are False) — loop now starts at the first finite bar.

#### AM.5 — Decision: freeze DeepSeek-leg evolution, make S22 operationally boring
Strategic call (2026-07-13): stop reflection-driven parameter tuning on the DeepSeek leg (it is tuning noise per AK.2/AL.2 evidence); keep it as a cheap canary with a kill criterion (30-day cost-adjusted expectancy ≤ 0 at end of July → freeze). Priorities instead: (1) S22 reliability — scheduled start so sessions can't be missed, intraday-leg exit logging, daily overnight-state check; let it reach 30-50 live trades before judging. (2) Close data-integrity holes: broker-side SL fills → no exit rows; BIOCON-style guardian duplicate-exit race; `reflect.py` mode filter. (3) Run `feature_study` on OFI now that rows are populating. No new strategy exploration until S22 has its verdict.
